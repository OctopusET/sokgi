...
pub enum Flag {
    ...
    Mtune(MachineSpec),
    Mabi(String),
    /// `-m16`/`-m32`/`-m64`/`-mx32`; holds `"16"`, `"32"`, `"64"`, `"x32"`.
    Mwidth(String),
    MfloatAbi(String),
    Mfpu(String),
    Mthumb(bool),
...
impl Flag {
    pub fn to_tokens(&self) -> Vec<String> {
        match self {
            ...
            Flag::Mabi(a) => vec![format!("-mabi={}", a)],
            Flag::Mwidth(w) => vec![format!("-m{}", w)],
            Flag::MfloatAbi(a) => vec![format!("-mfloat-abi={}", a)],
            Flag::Mfpu(u) => vec![format!("-mfpu={}", u)],
            Flag::Mthumb(true) => vec!["-mthumb".into()],
            Flag::Mthumb(false) => vec!["-mno-thumb".into()],
            Flag::Mendian(e) => vec![format!("-m{}-endian", e)],
            Flag::MhardFloat(true) => vec!["-mhard-float".into()],
            Flag::MhardFloat(false) => vec!["-msoft-float".into()],
            Flag::Mcmodel(c) => vec![format!("-mcmodel={}", c)],
            Flag::MalignDouble(true) => vec!["-malign-double".into()],
            Flag::MalignDouble(false) => vec!["-mno-align-double".into()],
            Flag::MgeneralRegsOnly(true) => vec!["-mgeneral-regs-only".into()],
            Flag::MgeneralRegsOnly(false) => vec!["-mno-general-regs-only".into()],
            Flag::MmsBitfields(true) => vec!["-mms-bitfields".into()],
            Flag::MmsBitfields(false) => vec!["-mno-ms-bitfields".into()],
